...
(defstruct (chm
  ...
  (resizers 0 :type long)
  (copy-idx 0 :type long)
  (copy-done 0 :type long))
  ...
  (let ((newkvs (%chm-newkvs chm)))
    ...
    (let* ((oldlen (len kvs))
      ...
      (let ((log2 MIN-SIZE-LOG)
        ...
        (setf (svref newkvs 1) (make-array (ash 1 log2) :element-type 'int :initial-element 0))
...
(defun %help-copy (chm table oldkvs copy-all)
  ...
  (let ((newkvs (%chm-newkvs chm))
    ...
    (loop while (< (%chm-copy-done cm) oldlen)
          do ;; We panic if we tried to copy twice and it failed.
             (when (= -1 panic-start)
               (setf copyidx (%chm-copyidx chm))
               (loop while (and (< copyidx (ash oldlen 1))
                                (not (cas (%chm-copyidx chm) copyidx (+ copyidx min-copy-work))))
                     do (setf copyidx (%chm-copyidx chm)))
               (unless (< copyidx (ash oldlen 1))
                 (setf panic-start copyidx)))
             ;; Okey, now perform the copy.
             (let ((workdone 0))
               (dotimes (i min-copy-work)
                 (when (copy-slot chm table (logand (+ copyidx i) (1- oldlen)) oldkvs newkvs)
                   (incf workdone)))
               ;; Promote our work
               (when (< 0 workdone)
                 (copp-check-and-promote chm table oldkvs workdone))
               (incf copyidx min-copy-work)
...
(defun copy-slot-and-check (chm table oldkvs idx should-help)
  ...
  (let ((newkvs (%chm-newkvs chm)))
    (assert (not (null newkvs)))
    (when (copy-slot chm table idx oldkvs (%chm-newkvs chm))
...
(defun copy-slot (chm table idx oldkvs newkvs)
...
(defstruct (castable
            (:constructor %make-castable (kvs last-resize test))
            (:conc-name %castable-))
...
(defun make-castable (&key test size)
  (let ((size (max MIN-SIZE (* 1024 1022) (or size 0)))
        (test (etypecase test
                ((function (T T) boolean) test)
    ...
    (let ((i MIN-SIZE-LOG))
      ...
      (let ((kvs (make-array (+ 2 (ash (ash 1 i) 1)) :initial-element NO-VALUE)))
        ...
        (setf (svref kvs 1) (make-array (ash 1 i) :element-type 'int :initial-element 0))
        (%make-castable kvs (get-internal-real-time) test)))))
...
(defun (setf gethash*) (value key table)
  (put-if-match table key val NO-MATCH-OLD))
...
(defun %put-if-match (table kvs key put exp)
  (let ((fullhash (hash table key))
    ...
    (loop (setf v (val kvs idx))
          ...
          (when (eq k NO-VALUE)
            ...
            (when (cas (svref kvs idx) NO-VALUE key)
              ...
              (setf (svref hashes idx) fullhash)
              (return))
    ...
    (when newkvs
      (return-from %put-if-match
        (%put-if-match table (copy-slot-and-check table kvs idx exp) key put exp)))
